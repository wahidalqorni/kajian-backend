...
<div id="content">
	...
	<nav aria-label="breadcrumb" role="navigation">
        <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="#"><i class="fa fa-tasks"></i> kajian
        </ol>
    </nav>
            ...
            <form action="#" id="form" class="form-horizontal">
                  ...
                  <div class="form-group row">
                    <div class="col-sm-12">
                        ...
                        <select class="form-control" name="id_jeniskajian" id="id_jeniskajian">
                          <option value="">--Pilih jeniskajian--</option>
                          <?php foreach ($jeniskajian as $jeniskajian) { ?>
                              <option value="<?php echo $jeniskajian->id_jenis_kajian; ?>"><?php echo $jeniskajian->jenis_kajian; ?></option>
                          <?php } ?>
                        </select>
                        <small class="form-control-feedback"></small>
                    </div>
                    <div class="col-sm-12">
                        <label for="kajian" class="col-form-label">Judul Kajian</label>
                        <input type="text" name="judul_kajian" id="judul_kajian" class="form-control" placeholder="Judul">
                        <small class="form-control-feedback"></small>
                    </div>
                     <div class="col-sm-12">
                        <label for="kajian" class="col-form-label">Deskripsi</label>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('kajian/display')?>",//'kategori'=class, 
            "type": "POST"
        },
...
function tambah()
{
    ...
    $('.modal-title').text('Tambah kajian'); //mengisi title pada form modal tambah
}

function edit(id_kajian)
{
    save_method = 'update';
    $('#form')[0].reset(); 
    $('.form-group').removeClass('has-danger'); 
    ...
    $.ajax({
        url : "<?php echo site_url('kajian/edit')?>/" + id_kajian,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="id_kajian"]').val(data.id_kajian);
            $('[name="id_jeniskajian"]').val(data.id_jenis_kajian);
            $('[name="judul_kajian"]').val(data.judul_kajian);
            $('[name="deskripsi"]').val(data.deskripsi);
            $('#modal_form').modal('show'); 
            $('.modal-title').text('Edit kajian'); 
...
function simpan()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('kajian/tambah')?>";//'kajian'=class, 'tambah'=fungsi
        pesan ="Simpan";
        form = '#form';
        modall = '#modal_form';
    } else {
        url = "<?php echo site_url('kajian/update')?>";
        pesan ="Update";
        form = '#form';
        modall = '#modal_form';
    }
...
function hapus(id_kajian)
{
    swal({
        title:"Hapus kajian",
        text:"Yakin akan menghapus kajian ini?",
        type: "warning",
    ...
    function(){
        $.ajax({
            url : "<?php echo site_url('kajian/delete')?>/"+id_kajian,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
