            ...
            <form action="#" id="form" class="form-horizontal">
                ...
                <div class="form-group row">
                    <div class="col-sm-6">
                        ...
                        <script type="text/javascript">tinymce.init({ selector : 'deskripsi', height : '250px', }); </script>
                        <small class="form-control-feedback"></small>
                    </div>
                    <div class="col-sm-6">
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('masjid2/display')?>",//'masjid2'=class, 
            "type": "POST"
        },
    ...
    $("#kota").change(function(){ // Ketika user mengganti atau memilih data provinsi
      ...
      $.ajax({
        ...
        url: "<?php echo base_url('index.php/masjid2/listKecamatan'); ?>", // Isi dengan url/path file php yang dituju
    ...
    $("#kecamatan").change(function(){ // Ketika user mengganti atau memilih data provinsi
      ...
      $.ajax({
        type: "POST", // Method pengiriman data bisa dengan GET atau POST
        url: "<?php echo base_url('index.php/masjid2/listKelurahan'); ?>", // Isi dengan url/path file php yang dituju
...
function tambah()
{
    ...
    $('.modal-title').text('Tambah masjid'); //mengisi title pada form modal tambah
}
...
function edit(id_masjid)
{
    ...
    $('#modal_form').modal('show');
    $.ajax({
        url : "<?php echo site_url('masjid2/edit')?>/" + id_masjid,
        type: "GET",
        dataType: "JSON",
...
function simpan()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('masjid2/tambah')?>";//'kecamatan'=class, 'tambah'=fungsi
        pesan ="Simpan";
        form = '#form';
        modall = '#modal_form';
    } else {
        url = "<?php echo site_url('masjid2/update')?>";
        pesan ="Update";
        form = '#form';
        modall = '#modal_form';
    }
...
function hapus(id_masjid)
{
    ...
    function(){
        $.ajax({
            url : "<?php echo site_url('masjid2/delete')?>/"+id_masjid,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
